...
class getTvCast extends StatefulWidget {
  final id;
  getTvCast({required this.id});

  @override
  _getTvCastState createState() => _getTvCastState();
}

class _getTvCastState extends State<getTvCast> {
  List cast = [];
  bool isLoading = true;
  ...
  load() async {
    ...
    Map c = await tmdb.v3.tv.getCredits(widget.id);
    setState(() {
      cast = c['cast'];
      isLoading = false;
    });
